...
export async function GET(request: Request) {
  ...
  const [transactions, total, tags] = await Promise.all([
    prisma.transaction.findMany({ where, orderBy: { occurredAt: 'desc' }, skip: offset, take: limit }),
  ...
  return NextResponse.json({ transactions, total, tags: tags.map((t) => t.tag).filter(Boolean) })
}
